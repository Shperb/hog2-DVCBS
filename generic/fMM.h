...
class fMM {
public:
	fMM(double frac = 0.5,double epsilon = 1.0):epsilon(epsilon) { forwardHeuristic = 0; backwardHeuristic = 0; env = 0; ResetNodeCount(); fraction = frac; }
...
uint64_t fMM<state, action, environment, priorityQueue>::GetNecessaryExpansions() const
{
	...
	for (unsigned int x = 0; x < forwardQueue.size(); x++)
	{
		...
		if ((data.where == kClosedList) && (fless(data.g+data.h, currentCost)) && fless(data.g / data.frac, currentCost))
			count++;
	}
